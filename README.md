...
4. Detect file and save ratio to txt with:

   ```
   python detect.py --source <your_folder_name> --weights weights/yolov5x_anime.pt --output <your_folder_ratio_ouput> --save-txt
   ```

5. Crop all result detected face

   ```
   python crop_img.py --input <original_folder> --ratio <ratio_folder> --margin <custom_extended_margin> (default: 50%)
   ```
...
7. Upscale using [REAL-ESRGAN](https://github.com/xinntao/Real-ESRGAN/releases/tag/v0.3.0)
   ...
   download the zip and extract it in the root project directory

   run:

   ```
   python filter_img.py --directory <orig_folder>/cropped/resized --execute upscale
   ```

   images result will be saved in `<orig_folder>/cropped/upscaled`

8. Filter using [Pilgram](https://github.com/akiomik/pilgram)

   there are lots of effects here you can use however you like, just go check out the repository.

   ```
   python filter_img.py --directory <orig_folder>/cropped/upscaled
   ```

   output will be saved in `<orig_folder>/cropped/upscaled/effects`
